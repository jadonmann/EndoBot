...
Congratulations! EndoBot will now run. On first start-up, EndoBot will acquire your server's ID, #general channel, and other basic information in order to build an internal directory for your server. Note that by default, all automated server communications generated by EndoBot will use your #general (or equivalent) channel to do so. You can change the output channel for each EndoBot feature by running the `!eb config` command within your server with moderator privileges.
